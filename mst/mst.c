...
int main ( int argc, char* argv[] ) {
    ...
    size_t graphNodeCount = adjMatrixToList(argv[1], &adjacencyList);//declares pointer to adjlist. initializes it by calling adjmatrixtolist, takes first arg passed, converts graph data to adjlist and returns # of nodes in graph
    ...
    for (unsigned iter=0; iter < graphNodeCount - 1; iter++) {
        ...
        graphNode_t minSource = -1;//-1 because have no been visited yet
        graphNode_t minDest = -1;//-1 because have no been visited yet

        for (graphNode_t source = 0; source < graphNodeCount; source++) {//does loop for amount of nodes
        
            if (parents[source] != -1) { // if already visited
                AdjacencyListNode* current = adjacencyList + source;//current is the node we are working with and source is added because it is the starting point
                
                while (current) {
                    if (parents[current -> graphNode] == -1 && current -> weight < minWeight) { //checks if edge has been visited and if its weight is below the min
                        minWeight = current -> weight;//updates minWeight as the weight of current
                        minSource = source;//updates minSource as the source
                        minDest = current -> graphNode;//updates minDest as the node of current
                    }
                    current = current -> next;//sets current as next
                }
            }
        }
        parents[minDest] = minSource; //found the min weight and set as minDest in parent.
    }

    // Using the fully populated parents array, print the edges in the MST.
    /* ... */
    for (graphNode_t i = 0; i < graphNodeCount; i++) {
        if (parents[i] != -1 && parents[i] != i) { // if visited and takes out dupelicate
            printf("%lu %lu\n", parents[i], i);//prints edgelist
        }
    }
