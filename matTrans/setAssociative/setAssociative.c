...
void accessData (
    ...
    size_t index;
    size_t recent_use_holder = program_counter;
    ...
    size_t lru = cache[hash][0].recent_use;
    for (int i = 0; i < E; i++) {
        if (cache[hash][i].valid == 1 && cache[hash][i].tag == tag) {
            cache[hash][i].recent_use = program_counter;
            (*hit_count)++;
            return;
        }
    }
    ...
    (*miss_count)++;
    for (int i = 0; i < E; i++) {
        if (cache[hash][i].recent_use < recent_use_holder) {
            recent_use_holder = cache[hash][i].recent_use;
            lru = i;
        }
    }

    // If cache set line already in use, then record an eviction
    if (!cache[hash][lru].valid) {
        cache[hash][lru].valid = 1;
    } else {
        (*eviction_count)++;
    }

    /* ... */
    cache[hash][lru].tag = tag;
    cache[hash][lru].recent_use = program_counter;
    return; 
}
